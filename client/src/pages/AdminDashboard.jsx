      ...
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-lg shadow-md">
        ...
        <div className="overflow-x-auto">
          <table className="w-full border-collapse bg-white shadow-md rounded-lg overflow-hidden">
            ...
            <tbody>
            {!users? (
        <p>No users found.</p>  // Show this if the users array is empty
      ) : (
              users.map((user) => (
                <tr key={user._id} className="border-b hover:bg-gray-100">
                  <td className="p-3"><input onChange={(e)=> setUpdateUser({...updateUser, username: e.target.value })} type="text" defaultValue={user.username} /></td>
                  <td className="p-3"><input onChange={(e)=> setUpdateUser({...updateUser, email: e.target.value})} type="email" defaultValue={user.email}/></td>
